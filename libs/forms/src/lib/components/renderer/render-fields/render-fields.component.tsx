...
export const RenderFields: FC<IRenderFieldsProps> = ({ fields, fieldPrefix = '' }: IRenderFieldsProps) => {
	const renderContentComponent = (field: IField) => {
		...
		if (field.min === 1 && field.max === 1) {
			...
			return <Component name={`${fieldPrefix}${field.slug}`} label={field.name} fieldConfiguration={fieldConfiguration} />
		}

		// TODO: Ditto
		return <RenderMultiple fieldPrefix={fieldPrefix} field={field}>
			{(index) => <Component name={`${fieldPrefix}${field.slug}.${index}`} label={field.name} fieldConfiguration={fieldConfiguration} />}
		</RenderMultiple>
	}
