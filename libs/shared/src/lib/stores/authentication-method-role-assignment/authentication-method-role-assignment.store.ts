...
import {
	IAuthenticationMethodRoleAssignment,
	IAuthenticationMethodRoleAssignmentStoreState,
	IAuthenticationMethodRoleAssignmentsResponse,
} from './authentication-method-role-assignment.types';

export const useAuthenticationMethodRoleAssignmentStore = create<IAuthenticationMethodRoleAssignmentStoreState>()(
	devtools(
		(set) => ({
			fetchAuthenticationMethodRoleAssignments: async (authenticationMethodId, params) => {
				...
				const [result, error] = await wrapApi(
					kyInstance
						.get(`/admin-api/v1/authentication-methods/${authenticationMethodId}/role-assignments`, {
							searchParams: {
								...(params || {}),
							},
						})
				...
				set(() => ({
					authenticationMethodRoleAssignments: result._embedded.authenticationMethodRoleAssignments,
					authenticationMethodRoleAssignmentsPagination: result._page,
			...
			fetchAuthenticationMethodRoleAssignment: async (authenticationMethodId, authenticationMethodRoleAssignmentId) => {
				...
				const [result, error] = await wrapApi(
					kyInstance
						.get(
							`/admin-api/v1/authentication-methods/${authenticationMethodId}/role-assignments/${authenticationMethodRoleAssignmentId}`
						)
			...
			createAuthenticationMethodRoleAssignment: async (authenticationMethodId, authenticationMethodRoleAssignment) => {
				...
				const [result, error] = await wrapApi(
					kyInstance
						.post(`/admin-api/v1/authentication-methods/${authenticationMethodId}/role-assignments`, {
